...
with open("input.txt") as f:
    ...
    for y in range(y_width):
        ...
        for x in range(x_width):
            ...
            if verticals_count % 2 == 1 and (x, y) not in loop_positions:
    ...
    for y in range(y_width):
        for x in range(x_width):
            if (x, y) in loop_positions:
                print(CL.WARNING + looplines[y][x] + CL.ENDC, end="")
            
            elif (x, y) in interior_coordinates:
                print(CL.OKBLUE + "■" + CL.ENDC, end="")
